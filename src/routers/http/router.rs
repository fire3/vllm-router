...
impl Router {
    ...
    pub async fn route_typed_request<T: GenerationRequest + serde::Serialize + Clone>(
        ...
        let response = RetryExecutor::execute_response_with_retry(
            ...
            |_: u32| async {
                ...
                // Acquire a per-worker slot before counting the request as
                // in-flight. Queueing here happens *after* consistent-hash
                // selection, so affinity is preserved while a busy worker is
                // protected from additional concurrent work.
                let slot_permit = match self.worker_registry.acquire_worker_slot(worker.url()).await
                {
                    Ok(permit) => permit,
                    Err(reject) => {
                        RouterMetrics::record_worker_admission_reject(
                            worker.url(),
                            reject.as_str(),
                        );
                        let mut response = reject.status_code().into_response();
                        response.extensions_mut().insert(reject);
                        return response;
                    }
                };
            ...
            |res, _attempt| {
                // Queue-full/timeout responses are generated by this router and
                // must not be retried into the same saturated worker.
                res.extensions().get::<AdmissionReject>().is_none()
    ...
    async fn send_typed_request<T: serde::Serialize>(
        ...
        slot_permit: Option<WorkerSlotPermit>,
    ) -> Response {
        ...
        if !is_stream {
            ...
            // The full response body has been read; the worker slot is free now.
            drop(slot_permit);
...
impl RouterTrait for Router {
    ...
    async fn route_transparent(
        ...
        // Gate the request on the selected worker so a saturated worker is
        // protected even on the transparent proxy path (e.g. /v1/responses).
        let slot_permit = match self.worker_registry.acquire_worker_slot(worker.url()).await {
            Ok(permit) => permit,
            Err(reject) => {
                RouterMetrics::record_worker_admission_reject(worker.url(), reject.as_str());
                let mut response = reject.status_code().into_response();
                response.extensions_mut().insert(reject);
                return response;
            }
        ...
        .await
        {
            Ok(response) => {
                ...
                let body = Body::from_stream(response.bytes_stream().map(move |chunk| {
                    // Holding the permit in the stream closure keeps it
                    // alive until the stream is consumed or dropped.
                    let _held = &slot_permit;
                    chunk
                }));
